  ...
  return (
    <li className="flex flex-col gap-3 px-6 py-5 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0 space-y-1.5">
        {showCategory && (
          <p className="text-base tracking-[0.2em] text-stone-500 uppercase">
            {item.category}
          </p>
        )}
        <p className="font-[family-name:var(--font-proposal-display)] text-3xl font-medium text-stone-900">
          {item.title}
        </p>
        {item.description && (
        ...
        <p className="text-base tracking-wide text-stone-500 uppercase">
          {showFullDateTime
            ? formatItemDateTime(item.scheduledAt)
            : formatItemTime(item.scheduledAt)}
        </p>
      </div>
      <p className="shrink-0 font-[family-name:var(--font-proposal-display)] text-2xl font-medium text-stone-800 tabular-nums">
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-stone-200/80 bg-white/80 shadow-sm shadow-stone-200/50 backdrop-blur-sm">
      <header className="border-b border-stone-100 px-6 py-4">
        <h2 className="font-[family-name:var(--font-proposal-display)] text-4xl font-medium tracking-wide text-stone-800">
          {category}
        </h2>
      </header>
...
function DaySection({
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-stone-200/80 bg-white/80 shadow-sm shadow-stone-200/50 backdrop-blur-sm">
      ...
      {items.length === 0 ? (
        <p className="px-6 py-8 text-center text-lg text-stone-500 italic">
      ...
      {(
        ...
        return (
          <button
            ...
            className={`rounded-full px-5 py-2 font-[family-name:var(--font-proposal-display)] text-lg tracking-wide transition-all ${
              isActive
